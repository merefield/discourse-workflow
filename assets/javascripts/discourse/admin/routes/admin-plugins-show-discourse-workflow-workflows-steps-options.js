...
export default class AdminPluginsShowDiscourseWorkflowWorkflowStepOptions extends DiscourseRoute {
  async model(params) {
    const allWorkflowStepOptions = await this.store.findAll("workflow-step-option", { position: params.id });  // this.modelFor("adminPlugins.show.discourse-workflow-workflows");
